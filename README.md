...
`manifests/slack/hermes-generated-manifest.json` is a captured copy of the Slack app manifest generated by Hermes Agent from inside the container. Use it as a reference when creating or updating the Slack app, then replace placeholder callback URLs such as `https://hermes-agent.local/slack/commands` with the public Slack webhook URL for your deployment.
